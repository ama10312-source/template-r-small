...
Open `./src/main.qmd` and hit the Preview button (top right corner) or press `Cmd + Shift + K` to compile the report. You should see an HTML report with some basic data analysis and a photo in `./bin/src/main.html`.
...
The file `./src/main.qmd` produces the final report. It generates an HTML file at `./bin/src/main.html`.

### External assets

A project may also use assets that are not generated by code (e.g., external images, bibliography, ...). These should be placed in `./assets` and committed to git. 
